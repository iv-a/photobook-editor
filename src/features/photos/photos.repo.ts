...
export const clearAllPhotosFromDB = async () => {
  await db.transaction("readwrite", db.photos, db.thumbs, db.meta, async () => {
...
export const decodeImageSize = async (file: Blob) => {
  if ("createImageBitmap" in window) {
    const bmp = await createImageBitmap(file);
    const res = { width: bmp.width, height: bmp.height };
    bmp.close?.();
...
export const makeThumbFromFile = async (file: Blob, max: number) => {
...
export const saveOneToDB = async (id: string, file: File, meta: PhotoMeta) => {
  const thumb = await makeThumbFromFile(file, 512);
  await db.transaction('readwrite', db.photos, db.thumbs, db.meta, async () => {
    await db.photos.put({ id, blob: file });
    await db.thumbs.put({ id, blob: thumb });
    await db.meta.put(meta);
